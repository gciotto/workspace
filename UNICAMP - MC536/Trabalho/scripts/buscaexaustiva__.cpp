...
int ap[] = {6, 866, 2004, 2371, 2404, 2453, 2482, 2577, 2984, 2995, 3434, 3445, 3620, 3660, 3662, 3674, 3706, 3729, 3796, 3808, 3828, 4237, 4448};
...
int main(int argc, char **argv) {
	...
	cout << "qq poha" << endl;
	for(;;) {
		vetorbuscado = fopen("vetor_lido.nor", "r");
		if(!vetorbuscado)
			continue;
...
//			return CANNOT_REMOVE_FILE;
		...
		out.close();
		rename("vetor_lido.ou", "vetor_lido.out");
		mede_tempo();
	}
	return 0;
}
...
int pre_processamento(int argc, char **argv, int &n, int &d, int &x, FILE * &mtx) {
	...
	fclose(mtx);
	//for(i = 0; i < 100; i++) cout << pesquisa[4976][i] << " ";
	return 0;
}
