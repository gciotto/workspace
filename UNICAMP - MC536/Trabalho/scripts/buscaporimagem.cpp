...
int k, d, x, elementos_centroide[K_MAX];
double matriz[K_MAX][MAX_ELEM_CENT][D_MAX];
...
int pre_processamento(int argc, char **argv) {
	...
	ifstream arq_cent;
	for(i = 0; i < k; i++) {
		end_cent = caminho + "centroide_" + str2i(d, 4) + "_" + str2i(i, 3) + ".txt";
		arq_cent.open(end_cent.c_str());
		for(j = 0; !arq_cent.eof(); j++)
			for(int cont = 0; cont < d; cont++)
				arq_cent >> matriz[i][j][cont];
		elementos_centroide[i] = j - 1;
		arq_cent.close();
	}
	return 0;
}
