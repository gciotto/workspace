...
static byte EE241_I2CAddress = (0&EE241_MAX_I2C_ADDR_MASK); /* current I2C address used */
...
/*
** ===================================================================
**     Method      :  EE241_Test (component 24AA_EEPROM)
**     Description :
...
byte EE241_Test(void)
{
  uint8_t res, val, data[16];

  res = EE241_WriteByte(0x0000, 0);
  if (res != ERR_OK) {
    Err();
    return res;
  }
  res = EE241_WriteByte(0x0001, 1);
  if (res != ERR_OK) {
    Err();
    return res;
  }
  res = EE241_WriteByte(0x0002, 2);
  if (res != ERR_OK) {
    Err();
    return res;
  }
  res = EE241_WriteByte(0x0003, 3);
  if (res != ERR_OK) {
    Err();
    return res;
  }
  res = EE241_WriteByte(0x0004, 4);
  if (res != ERR_OK) {
    Err();
    return res;
  }
  res = EE241_WriteByte(0x0010, 5);
  if (res != ERR_OK) {
    Err();
    return res;
  }
  ...
  res = EE241_ReadBlock(0x10, data, sizeof(data));
  if (res != ERR_OK) {
    Err();
    return res;
  }
  ...
  res = EE241_ReadBlock(EE241_PAGE_SIZE-5, data, sizeof(data));
  if (res != ERR_OK) {
    Err();
    return res;
  }
  if (strcmp((char*)"Hello World!", (char*)data) != 0) return ERR_FAULT;
#if EE241_DEVICE_ID==EE241_DEVICE_ID_1025
  /* testing writing to second bank */
  res = EE241_WriteBlock(0x10005, (uint8_t*)"Hello bank 1!", sizeof("Hello bank 1!"));
  if (res != ERR_OK) {
    Err();
    return res;
  }
  res = EE241_ReadBlock(0x10005, data,(uint8_t*) sizeof(data));
  if (res != ERR_OK) {
    Err();
    return res;
  }
...
/*
** ===================================================================
**     Method      :  EE241_SelectDevice (component 24AA_EEPROM)
**     Description :
...
** ===================================================================
*/
byte EE241_SelectDevice(byte addrI2C)
{
  if (addrI2C>EE241_MAX_I2C_ADDR_MASK) {
