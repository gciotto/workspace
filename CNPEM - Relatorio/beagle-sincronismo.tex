...
\textit{patch}, chamado \path{RT_PREEMPT} e disponível em
...
de \textit{host order} para \textit{network byte order}, por exemplo \path{0xc0a80216} para \path{192.168.2.22}, \path{INADDR_SEND} é o
endereço para o qual deseja-se enviar o datagramas e \path{CONNECT_PORT} é a
porta. \path{connect()}, além de definir tais características, também ativa a conexão no \textit{socket}. O envio de pacotes é
...
\centering
	\begin{subfigure}[t]{0.5\textwidth}
		\centering
		\includegraphics[scale=0.26]{image/tek_com_threads}
	...
	\begin{subfigure}[t]{0.5\textwidth}
		\centering
		\includegraphics[scale=0.26]{image/tek_pru}
