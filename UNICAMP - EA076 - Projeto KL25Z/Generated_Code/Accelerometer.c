...
#define Accelerometer_CPU_IS_LITTLE_ENDIAN 1 /* Cpu is little endian */
...
{
  static const uint8_t addr = Accelerometer_OUT_X_MSB;

  return GI2C2_ReadAddress(Accelerometer_I2C_ADDR, (uint8_t*)&addr, sizeof(addr), &xyz[0], 3);
}
...
uint8_t Accelerometer_Init(void)
{
  ...
  return GI2C2_WriteByteAddress8(Accelerometer_I2C_ADDR, Accelerometer_CTRL_REG_1, Accelerometer_ACTIVE_BIT_MASK); /* enable device */
}
...
void Accelerometer_CalibrateX1g(void)
{
  ...
  for (i=0; i<8; i++) {
   X += ((int16_t)Accelerometer_MeasureGetRawX() >> 2);
   Y += ((int16_t)Accelerometer_MeasureGetRawY() >> 2);
   Z += ((int16_t)Accelerometer_MeasureGetRawZ() >> 2);
  }
...
uint16_t Accelerometer_MeasureGetRawX(void)
{
  ...
  if(GI2C2_ReadAddress(Accelerometer_I2C_ADDR, (uint8_t*)&addr, sizeof(addr), &val.buf[0], sizeof(val.buf))!=ERR_OK) {
    return 0; /* failure */
  }
#if Accelerometer_CPU_IS_LITTLE_ENDIAN
...
uint16_t Accelerometer_MeasureGetRawY(void)
{
  ...
  static const uint8_t addr = Accelerometer_OUT_Y_MSB;
...
uint16_t Accelerometer_MeasureGetRawZ(void)
{
  ...
  static const uint8_t addr = Accelerometer_OUT_Z_MSB;
...
int16_t Accelerometer_GetX(void)
{
  ...
  value = (int16_t)Accelerometer_MeasureGetRawX() >> 2; /* remove 2 bits (16bit to 14bit) */
...
int16_t Accelerometer_GetY(void)
{
  ...
  value = (int16_t)Accelerometer_MeasureGetRawY() >> 2; /* remove 2 bits (16bit to 14bit) */
...
int16_t Accelerometer_GetZ(void)
{
  ...
  value = (int16_t)Accelerometer_MeasureGetRawZ() >> 2; /* remove 2 bits (16bit to 14bit) */
...
uint8_t Accelerometer_SetFastMode(bool on)
{
  ...
  res = GI2C2_ReadByteAddress8(Accelerometer_I2C_ADDR, Accelerometer_CTRL_REG_1, &val);
  if (res!=ERR_OK) {
    return res;
  }
  if (on) {
    val |= Accelerometer_F_READ_BIT_MASK; /* enable F_READ: Fast read mode, data format limited to single byte (auto increment counter will skip LSB) */
  } else {
    val &= ~Accelerometer_F_READ_BIT_MASK; /* disable F_READ: Fast read mode, data format limited to single byte (auto increment counter will skip LSB) */
  }
  return GI2C2_WriteByteAddress8(Accelerometer_I2C_ADDR, Accelerometer_CTRL_REG_1, val);
}
...
uint8_t Accelerometer_Enable(void)
{
  ...
  if (val&Accelerometer_ACTIVE_BIT_MASK) {
    return ERR_OK; /* already enabled */
  }
  WAIT3_Waitms(1); /* for unknown reasons, need to wait for 1 ms after reading the register */
  val |= Accelerometer_ACTIVE_BIT_MASK; /* enable device */
  return GI2C2_WriteByteAddress8(Accelerometer_I2C_ADDR, Accelerometer_CTRL_REG_1, val);
}
...
uint8_t Accelerometer_Disable(void)
{
  ...
  if ((val&Accelerometer_ACTIVE_BIT_MASK)==0) {
    return ERR_OK; /* already disabled */
  }
  WAIT3_Waitms(1); /* for unknown reasons, need to wait for 1 ms after reading the register */
  val &= ~Accelerometer_ACTIVE_BIT_MASK; /* disable device */
  return GI2C2_WriteByteAddress8(Accelerometer_I2C_ADDR, Accelerometer_CTRL_REG_1, val);
}
...
uint8_t Accelerometer_isEnabled(bool *isEnabled)
{
  ...
  *isEnabled = (val&Accelerometer_ACTIVE_BIT_MASK)!=0; /* TRUE if bit is set, FALSE otherwise */
  return ERR_OK;
}
...
{
  return GI2C2_ReadByteAddress8(Accelerometer_I2C_ADDR, Accelerometer_WHO_AM_I, value);
}
...
{
  if (GI2C2_ReadByteAddress8(Accelerometer_I2C_ADDR, addr, val)!=ERR_OK) {
    return ERR_FAILED;
  }
  return ERR_OK;
}
...
{
  return GI2C2_WriteByteAddress8(Accelerometer_I2C_ADDR, addr, val);
}
...
uint8_t Accelerometer_SwReset(void)
{
  ...
  res = GI2C2_ReadByteAddress8(Accelerometer_I2C_ADDR, Accelerometer_CTRL_REG_2, &val);
  if (res!=ERR_OK) {
    return res;
  }
  val |= Accelerometer_CTRL_REG_2_MASK_RST; /* reset device */
  res = GI2C2_WriteByteAddress8(Accelerometer_I2C_ADDR, Accelerometer_CTRL_REG_2, val);
  if (res!=ERR_OK) {
    return res;
  }
  WAIT3_Waitms(1); /* wait for one ms after sending the software reset */
  return ERR_OK;
}
