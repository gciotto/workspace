...
{
  if (I2CAccel_SelectSlaveDevice(GI2C2_deviceData.handle, LDD_I2C_ADDRTYPE_7BITS, i2cAddr)!=ERR_OK) {
    return ERR_FAILED;
  }
  return ERR_OK;
}
...
byte GI2C2_ReadBlock(void* data, word dataSize, GI2C2_EnumSendFlags flags)
{
  ...
  for(;;) { /* breaks */
    ...
    res = I2CAccel_MasterReceiveBlock(GI2C2_deviceData.handle, data, dataSize, flags==GI2C2_SEND_STOP?LDD_I2C_SEND_STOP:LDD_I2C_NO_SEND_STOP);
    if (res!=ERR_OK) {
      break; /* break for(;;) */
    }
...
byte GI2C2_WriteBlock(void* data, word dataSize, GI2C2_EnumSendFlags flags)
{
  ...
  for(;;) { /* breaks */
    ...
    res = I2CAccel_MasterSendBlock(GI2C2_deviceData.handle, data, dataSize, flags==GI2C2_SEND_STOP?LDD_I2C_SEND_STOP:LDD_I2C_NO_SEND_STOP);
    if (res!=ERR_OK) {
      break; /* break for(;;) */
    }
...
byte GI2C2_ReadAddress(byte i2cAddr, byte *memAddr, byte memAddrSize, byte *data, word dataSize)
{
  ...
  for(;;) { /* breaks */
    ...
    GI2C2_deviceData.dataTransmittedFlg = FALSE;
    res = I2CAccel_MasterSendBlock(GI2C2_deviceData.handle, memAddr, memAddrSize, LDD_I2C_NO_SEND_STOP);
    if (res!=ERR_OK) {
      break; /* break for(;;) */
    }
    do { /* Wait until data is sent */
    ...
    /* receive data */
    GI2C2_deviceData.dataReceivedFlg = FALSE;
    res = I2CAccel_MasterReceiveBlock(GI2C2_deviceData.handle, data, dataSize, LDD_I2C_SEND_STOP);
    if (res!=ERR_OK) {
      break; /* break for(;;) */
    }
    do { /* Wait until data is received */
...
{
  static byte writeBuf[GI2C2_WRITE_BUFFER_SIZE];
  byte *p;
  word i;
  byte res = ERR_OK;
  ...
  for(;;) { /* breaks */
    ...
    GI2C2_deviceData.dataTransmittedFlg = FALSE;
    if (I2CAccel_MasterSendBlock(GI2C2_deviceData.handle, writeBuf, i, LDD_I2C_SEND_STOP)!=ERR_OK) {
      break; /* break for(;;) */
    }
    do { /* Wait until data is sent */
    } while (!GI2C2_deviceData.dataTransmittedFlg);
...
{
  GI2C2_deviceData.handle = I2CAccel_Init(&GI2C2_deviceData);
...
{
  I2CAccel_Deinit(&GI2C2_deviceData);
}
...
{
  return GI2C2_WriteAddress(i2cAddr, &memAddr, sizeof(memAddr), &data, 1);
}
...
byte GI2C2_ScanDevice(byte i2cAddr)
{
  ...
  for(;;) { /* breaks */
    /* send device address */
    GI2C2_deviceData.dataTransmittedFlg = FALSE;
    res = I2CAccel_MasterReceiveBlock(GI2C2_deviceData.handle, &dummy, 1, LDD_I2C_SEND_STOP);
    if (res!=ERR_OK) {
      break; /* break for(;;) */
    }
    do { /* Wait until data is sent */
    ...
    errMask = 0;
    (void)I2CAccel_GetError(GI2C2_deviceData.handle, &errMask);
