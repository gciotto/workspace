		...
		struct bsmp_message *response)
{
	...
	send_buf.data[1] = IS_BIG_ENDIAN ? request->payload_size >> 8 : request->payload_size;
	send_buf.data[2] = IS_BIG_ENDIAN ? request->payload_size : request->payload_size >> 8;
	...
	response->payload_size = 	IS_BIG_ENDIAN ?
			(recv_buf.data[1] << 8) | recv_buf.data[2] :
			(recv_buf.data[2] << 8) | recv_buf.data[1];
		...
		uint16_t *len)
{
	...
	request.payload[0] = curve->id;
	request.payload[1] = IS_BIG_ENDIAN ? offset >> 8 : offset;
	request.payload[2] = IS_BIG_ENDIAN ? offset : offset >> 8;
